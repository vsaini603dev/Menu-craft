...
import React, { forwardRef, memo, useCallback, useImperativeHandle, useMemo, useRef, useState, createElement as _createElement } from 'react';
import { useBottomSheetModalInternal } from '../../hooks';
import { print } from '../../utilities';
import { id } from '../../utilities/id';
import BottomSheet from '../bottomSheet';
import { DEFAULT_ENABLE_DISMISS_ON_CLOSE, DEFAULT_STACK_BEHAVIOR, MODAL_STATUS } from './constants';
...
function BottomSheetModalComponent(props, ref) {
  ...
  const statusRef = useRef(MODAL_STATUS.INITIAL);
  ...
  const handleSnapToIndex = useCallback((...args) => {
    if (statusRef.current === MODAL_STATUS.MINIMIZED || statusRef.current === MODAL_STATUS.MINIMIZING) {
      return;
    }
    bottomSheetRef.current?.snapToIndex(...args);
  }, []);
  const handleSnapToPosition = useCallback((...args) => {
    if ([MODAL_STATUS.MINIMIZED, MODAL_STATUS.MINIMIZING, MODAL_STATUS.DISMISSED, MODAL_STATUS.DISMISSING].includes(statusRef.current)) {
      return;
    }
  ...
  const handleClose = useCallback((...args) => {
    if ([MODAL_STATUS.MINIMIZED, MODAL_STATUS.MINIMIZING, MODAL_STATUS.DISMISSED, MODAL_STATUS.DISMISSING, MODAL_STATUS.CLOSED].includes(statusRef.current)) {
      return;
    }
  ...
  const handleDismiss = useCallback(function handleDismiss(animationConfigs) {
     ...
     */
    if ([MODAL_STATUS.CLOSED, MODAL_STATUS.MINIMIZED].includes(statusRef.current) || statusRef.current === MODAL_STATUS.DISMISSING && currentIndexRef.current === -1) {
  ...
  const handleRestore = useCallback(function handleRestore() {
     ...
     */
    const minimizedOrGoingToBe = [MODAL_STATUS.MINIMIZING, MODAL_STATUS.MINIMIZED].includes(statusRef.current);
  ...
  const handlePortalOnUnmount = useCallback(function handlePortalOnUnmount() {
     ...
     */
    if (statusRef.current === MODAL_STATUS.MINIMIZED || statusRef.current === MODAL_STATUS.DISMISSED || currentIndexRef.current === -1) {
      unmount();
      return;
    }
    statusRef.current = MODAL_STATUS.DISMISSING;
  ...
  const handleBottomSheetOnChange = useCallback(function handleBottomSheetOnChange(_index, _position, _type) {
    if (__DEV__) {
      print({
        component: 'BottomSheetModal',
    ...
    statusRef.current = _index === -1 ? MODAL_STATUS.MINIMIZED : MODAL_STATUS.PRESENTED;
    if (_providedOnChange) {
      _providedOnChange(_index, _position, _type);
    }
  }, [_providedOnChange]);
  const handleBottomSheetOnAnimate = useCallback((fromIndex, toIndex, fromPosition, toPosition) => {
    if (__DEV__) {
      print({
        component: 'BottomSheetModal',
     ...
     */
    const currentStatusIsDismissingOrMinimizing = [MODAL_STATUS.DISMISSING, MODAL_STATUS.MINIMIZING].includes(statusRef.current);
  ...
  const handleBottomSheetOnClose = useCallback(function handleBottomSheetOnClose() {
    ...
    if (statusRef.current === MODAL_STATUS.DISMISSING) {
      ...
      statusRef.current = enableDismissOnClose ? MODAL_STATUS.DISMISSED : MODAL_STATUS.CLOSED;
    }
  ...
  // render
  return mount ? /*#__PURE__*/_jsx(Portal, {
    name: key,
    hostName: hostName,
    ...
    children: /*#__PURE__*/_jsx(ContainerComponent, {
      children: /*#__PURE__*/_createElement(BottomSheet, {
        ...bottomSheetProps,
        ref: bottomSheetRef,
        key: key,
        index: index,
...
const BottomSheetModal = /*#__PURE__*/memo(/*#__PURE__*/forwardRef(BottomSheetModalComponent));
