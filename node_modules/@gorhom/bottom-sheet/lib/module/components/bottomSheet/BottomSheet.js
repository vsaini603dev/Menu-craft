...
import { DEFAULT_ACCESSIBILITY_LABEL, DEFAULT_ACCESSIBILITY_ROLE, DEFAULT_ACCESSIBLE, DEFAULT_ANIMATE_ON_MOUNT, DEFAULT_DYNAMIC_SIZING, DEFAULT_ENABLE_BLUR_KEYBOARD_ON_GESTURE, DEFAULT_ENABLE_CONTENT_PANNING_GESTURE, DEFAULT_ENABLE_OVER_DRAG, DEFAULT_ENABLE_PAN_DOWN_TO_CLOSE, DEFAULT_KEYBOARD_BEHAVIOR, DEFAULT_KEYBOARD_BLUR_BEHAVIOR, DEFAULT_KEYBOARD_INDEX, DEFAULT_KEYBOARD_INPUT_MODE, DEFAULT_OVER_DRAG_RESISTANCE_FACTOR, INITIAL_VALUE } from './constants';
...
const BottomSheetComponent = /*#__PURE__*/forwardRef(function BottomSheet(props, ref) {
  ...
  // conditional
  const didAnimateOnMount = useSharedValue(!animateOnMount || _providedIndex === -1);
  const isLayoutCalculated = useDerivedValue(() => {
    ...
    if (containerHeight !== null && containerHeight !== undefined && containerHeight !== INITIAL_LAYOUT_VALUE) {
  ...
  const getEvaluatedPosition = useCallback(function getEvaluatedPosition(source) {
     ...
     */
    if (source === ANIMATION_SOURCE.KEYBOARD && keyboardBlurBehavior === KEYBOARD_BLUR_BEHAVIOR.restore && keyboardStatus === KEYBOARD_STATUS.HIDDEN && animatedContentGestureState.value !== State.ACTIVE && animatedHandleGestureState.value !== State.ACTIVE) {
  ...
  const evaluatePosition = useCallback(function evaluatePosition(source, animationConfigs) {
    ...
    if (!didAnimateOnMount.value) {
       ...
       */
      if (animationStatus === ANIMATION_STATUS.RUNNING && (isForcedClosing || nextIndex === -1)) {
        return;
      }
      /**
