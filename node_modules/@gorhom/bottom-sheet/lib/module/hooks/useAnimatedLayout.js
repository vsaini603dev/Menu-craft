 ...
 */
export function useAnimatedLayout(containerLayoutState, topInset, bottomInset, modal, shouldOverrideHandleHeight) {
  //#region  variables
  const verticalInset = useMemo(() => topInset + bottomInset, [topInset, bottomInset]);
  ...
  //#region effects
  useAnimatedReaction(() => state.value.rawContainerHeight, (result, previous) => {
    if (result === previous) {
      return;
    }
  ...
  useAnimatedReaction(() => containerLayoutState?.get().height, (result, previous) => {
