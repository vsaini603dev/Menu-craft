...
export const useGestureEventsHandlersDefault = () => {
  ...
  const handleOnStart = useCallback(function handleOnStart(__, {
    translationY
  }) {
    'worklet';
  ...
  const handleOnChange = useCallback(function handleOnChange(source, {
     ...
     */
    if (source === GESTURE_SOURCE.CONTENT && scrollableIsRefreshable && animatedPosition.value === highestSnapPoint) {
      return;
    }
     ...
     */
    const negativeScrollableContentOffset = context.value.initialPosition === highestSnapPoint && source === GESTURE_SOURCE.CONTENT || !context.value.isScrollablePositionLocked ? scrollableContentOffsetY * -1 : 0;
     ...
     */
    if (enableOverDrag) {
      if ((source === GESTURE_SOURCE.HANDLE || scrollableType === SCROLLABLE_TYPE.VIEW) && draggedPosition < highestSnapPoint) {
  ...
  }, [enablePanDownToClose, isInTemporaryPosition, animatedScrollableState, animatedDetentsState, animatedKeyboardState, animatedPosition, animatedLayoutState, animateToPosition, context]);
