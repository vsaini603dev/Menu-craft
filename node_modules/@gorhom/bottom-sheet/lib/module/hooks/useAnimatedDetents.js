 ...
 */
export const useAnimatedDetents = (detents, layoutState, enableDynamicSizing, maxDynamicContentSize, detached, $modal, bottomInset = 0) => {
  const state = useDerivedValue(() => {
    ...
    let _normalizedDetents = _detents.map(snapPoint => normalizeSnapPoint(snapPoint, containerHeight));
